...
#Importing the OS , Pytest , extras, wait adn exception

# Setting up the environment here
@pytest.fixture()
def set_up():
    option=webdriver.ChromeOptions()
    option.add_argument("--disable-notifications")    #Disabling the notification box
    driver=webdriver.Chrome(option)                   #passing the chrome option into driver
    driver.get("https://opensource-demo.orangehrmlive.com/web/index.php/auth/login") #URL to the wbpage to test
    driver.maximize_window()                           #setting the window siz to maximum
    try:
         alert=driver.switch_to.alert
         alert.accept()                                #handling the alerts that might occur
    except NoAlertPresentException:
        print("No Alert")
    return driver                                      # returning the driver

#This fixture runs automatically after the test run
@pytest.fixture(autouse=True)
def tear_down(set_up):
    driver=set_up
    yield
    driver.quit()   # To stop the test run
